      ...
      <div className="flex items-center gap-4">
        {!user && (
          <>
            ...
            <Link to="/register/employee" className="btn btn-sm bg-teal-400 text-white">Join as Employee</Link>
            <Link to="/register/hr" className="btn btn-sm bg-teal-400 text-white">Join as HR</Link>
          </>
        )}
        {user && (
          <div className="dropdown dropdown-end">
            <label tabIndex={0} className="btn btn-sm btn-outline bg-teal-400 rounded-full">
              {user.email}
            </label>
